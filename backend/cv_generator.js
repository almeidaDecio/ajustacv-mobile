...
const FIXED_METRICS = [
  '5 novos clientes no setor financeiro (Santander e Banco do Brasil) e crescimento de 10x na volumetria processada pelos clientes',
...
const METRICS_CHECKS = [
  b => /10x|10 vez|dez vez|volumetria/i.test(b),
  b => /50 nov|cinquenta/i.test(b),
  b => /15\+|quinze|15 entrev/i.test(b),
  b => /2 flux|dois flux|comprovação fiscal|sensoriamento remoto/i.test(b),
];
...
6. REGRA OBRIGATÓRIA — RESULTADOS QUANTITATIVOS DA SOFTFOCUS:
   ...
   ATENÇÃO: Se você reescrever qualquer um desses bullets perdendo
   os números ou os nomes dos clientes, o CV será rejeitado.
   Use exatamente: "10x", "50", "15+", "2 fluxos".
...
function generateHTML(cv, ollamaResult) {
  ...
  // Limpa prefixos de bullet que o Ollama às vezes adiciona (•, -, –, *)
